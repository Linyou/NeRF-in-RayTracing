...
class BVH:
    ...
    def __init__(self, object_list):
        ...
        self.bvh_file = './lego_bvh.npy'

    def save_(self):
        ''' Saves the bvh to a file '''
        bvh_obj_id = self.bvh_obj_id.to_numpy()
        bvh_left_id = self.bvh_left_id.to_numpy()
        bvh_right_id = self.bvh_right_id.to_numpy()
        bvh_next_id = self.bvh_next_id.to_numpy()
        bvh_min = self.bvh_min.to_numpy()
        bvh_max = self.bvh_max.to_numpy()

        save_dict = {
            'bvh_obj_id': bvh_obj_id,
            'bvh_left_id': bvh_left_id,
            'bvh_right_id': bvh_right_id,
            'bvh_next_id': bvh_next_id,
            'bvh_min': bvh_min,
            'bvh_max': bvh_max,
        }
        np.save(self.bvh_file, save_dict)

    def load_(self):
        ''' Loads the bvh from a file '''
        save_dict = np.load(self.bvh_file, allow_pickle=True).item()
        self.bvh_obj_id.from_numpy(save_dict['bvh_obj_id'])
        self.bvh_left_id.from_numpy(save_dict['bvh_left_id'])
        self.bvh_right_id.from_numpy(save_dict['bvh_right_id'])
        self.bvh_next_id.from_numpy(save_dict['bvh_next_id'])
        self.bvh_min.from_numpy(save_dict['bvh_min'])
        self.bvh_max.from_numpy(save_dict['bvh_max'])
